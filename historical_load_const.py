...
# 공사 API Endpoints (조달청 대행 + 자체 발주)
API_ENDPOINTS = [
    'https://apis.data.go.kr/1230000/ao/CntrctInfoService/getCntrctInfoListCnstwkPPSSrch',
    'https://apis.data.go.kr/1230000/ao/CntrctInfoService/getCntrctInfoListCnstwkSrch'
]
...
def download_for_date(date_str, api_url):
    print(f"[공사] {date_str} 조회 중... ({'중앙' if 'PPS' in api_url else '자체'})")
...
def main():
    print("==========================================================")
    print(" 🚀 조달청 [공사] 계약 1차 초기 적재 (Historical Load)")
    print("    - 대상: 공사계약현황 검색기능 API (getCntrctInfoListCnstwk PPSSrch + Srch)")
    ...
    print(f"총 [{len(dates)}일] 간의 과거 공사 데이터를 양대 API에서 병렬 다운로드합니다...")
    ...
    print(f"\\n✅ 2. 로컬 SQLite DB ({DB_PATH}) -> 'cnstwk_cntrct' (공사) 테이블 저장 시작...")
    if all_data:
        ...
        print(f"   - [공사] 전국 {n_before:,} → 부산 {len(df):,}건 필터 적용")
        
        df.to_sql('cnstwk_cntrct', conn, if_exists='replace', index=False)
        print(f"   - [공사] 총 {len(df):,}건 적재 성공.")
        conn.close()
    else:
    ...
    end_time = time.time()
    print("==========================================================")
    print(f"🎉 공사계약 로드 작업 성공 종료. (총 소요시간: {end_time - start_time:.1f}초)")
    print("==========================================================")
