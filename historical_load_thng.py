...
# 물품 API Endpoints (조달청 대행 + 자체 발주)
API_ENDPOINTS = [
    'https://apis.data.go.kr/1230000/ao/CntrctInfoService/getCntrctInfoListThngPPSSrch',
    'https://apis.data.go.kr/1230000/ao/CntrctInfoService/getCntrctInfoListThngSrch'
]
...
def download_for_date(date_str, api_url):
    print(f"[물품] {date_str} 조회 중... ({'중앙' if 'PPS' in api_url else '자체'})")
...
def main():
    print("==========================================================")
    print(" 🚀 조달청 [물품] 계약 1차 초기 적재 (Historical Load)")
    print("    - 대상: 물품계약현황 검색기능 API (getCntrctInfoListThngPPSSrch)")
    ...
    print(f"총 [{len(dates)}일] 간의 과거 물품 데이터를 양대 API에서 병렬 다운로드합니다...")
    ...
    # DB 저장
    print(f"\n✅ 2. 로컬 SQLite DB ({DB_PATH}) -> 'thng_cntrct' (물품) 테이블 저장 시작...")
    if all_data:
    ...
    end_time = time.time()
    print("==========================================================")
    print(f"🎉 물품계약 로드 작업 성공 종료. (총 소요시간: {end_time - start_time:.1f}초)")
    print("==========================================================")
