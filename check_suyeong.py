...
def find_agency_stats(keyword, start_dt, end_dt):
    ...
    print(f"🔎 [{keyword}] 검색 결과 ({len(target_codes)}개 코드 발견)")
    ...
    def calc_stats(table_name, date_col, is_shop=False):
        if is_shop:
            df = pd.read_sql(f"SELECT dminsttCd, prdctAmt, cntrctCorpBizno FROM {table_name} WHERE {date_col} >= '{start_dt}' AND {date_col} <= '{end_dt}'", conn_pr)
            df['target_cd'] = df['dminsttCd'].astype(str).str.strip()
            df_target = df[df['target_cd'].isin(target_codes)]
        else:
            df = pd.read_sql(f"SELECT cntrctInsttCd, totCntrctAmt, thtmCntrctAmt, corpList, dminsttList FROM {table_name} WHERE {date_col} >= '{start_dt}' AND {date_col} <= '{end_dt}'", conn_pr)
        ...
        for i, row in df_target.iterrows():
            if is_shop:
                ...
                if pd.isna(amt) or amt == 0: amt = float(row.get('thtmCntrctAmt', 0))
                b_nos = []
    ...
    cnt_s, tot_s, loc_s = calc_stats('servc_cntrct', 'cntrctDate', False)
    cnt_t, tot_t, loc_t = calc_stats('thng_cntrct', 'cntrctDate', False)
    ...
    print(f"📦 [물품 통합] {cnt_t+cnt_p}건 | 발주: {tot_t+tot_p:,.0f}원 / 수주: {loc_t+loc_p:,.0f}원 ({(loc_t+loc_p)/(tot_t+tot_p)*100 if (tot_t+tot_p)>0 else 0:.1f}%)")
    print(f"   ├─ 일반물품: {cnt_t}건 | 발주: {tot_t:,.0f}원 / 수주: {loc_t:,.0f}원")
    print(f"   └─ 종합쇼핑몰: {cnt_p}건 | 발주: {tot_p:,.0f}원 / 수주: {loc_p:,.0f}원")
...
if __name__ == '__main__':
    find_agency_stats('부산광역시 수영구', '2026-01-01', '2026-01-31')
