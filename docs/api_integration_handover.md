...
## 5. 5/27 챗봇 DB 적재 경보 문자 오탐 원인 및 보완 조치 (2026-05-27 핫픽스)

어제(5/26) 조치 완료 후에도 오늘 아침 9시에 일부 경보 문자가 다시 발송된 상세 원인 및 이를 완전히 해결하기 위한 추가 복구 조치 내역입니다.

### 5.1 추가 분석 및 근본 원인
1. **경보 감시 목록 불일치 (오탐 발생)**:
   - 크론탭 실행 대상은 실존하는 `import_policy_company.py`로 고쳐서 오늘 아침 정상 완료되었으나, 경보 판단 스크립트인 `alert_check.py` 내의 필수 감시 배열에는 여전히 과거에 쓰이던 `import_direct_production_cert_api`가 하드코딩되어 있었습니다. 이로 인해 성공 기록을 찾지 못해 "오늘 미실행" 지연 오탐 경보가 발송되었습니다.
2. **신규 0건 적재 시 비정상 종료 처리**:
   - `bootstrap_master_data.py` 실행 시 소스 데이터가 이미 다 이전되어 신규로 추가될 면허/제품 건수가 0건일 경우, `FAIL` 로그를 찍으며 `exit 1`로 실패 종료되도록 스크립트 가드가 짜여 있었습니다. 이로 인해 오늘 새벽 이관 작업이 `failed` 판정을 받았습니다.
3. **SQLite3 데이터베이스 락(Locked) 발생**:
   - `import_policy_company.py`는 부산업체 46,000여 건 전체의 사업자번호를 루프 돌며 SMPP API 조회를 호출하는 헤비한 배치입니다. (수동 기동 시 약 2.5시간 소요)
   - 05:35에 시작된 이 배치가 SQLite DB 트랜잭션을 장시간 물고 돌면서, 05:45에 실행된 `import_mas_product_api.py`(종합쇼핑몰 수집)가 DB 락에 걸려 `database is locked` 오류를 발생시키며 실패했습니다.

### 5.2 해결 조치 내역 (5/27 반영 완료)
1. **감시 스크립트 (`alert_check.py`) 하드코딩 최신화**:
   - `alert_check.py` 내에서 감시할 핵심 필수 배치 목록을 실제 구동 파일 및 명칭과 일치하도록 최신화했습니다.
   - **수정 전**: `bootstrap_master_data`, `mas_api_incremental`, `certified_product_api_incremental`
   - **수정 후**: `bootstrap_master`, `import_certified_product_api`, `import_innovation_product_api`, `mas_api_incremental`
2. **`bootstrap_master_data.py` 신규 0건 정상화 처리**:
   - 데이터베이스가 최신 상태로 유지되어 신규 적재 데이터 건수가 0건이더라도 에러(`exit 1`)가 아닌 성공(`exit 0`)으로 정상 리턴하도록 핫픽스를 적용하여 오탐 경보를 예방했습니다.
3. **크론탭 구동 주기 분리 (SQLite DB Lock 해결)**:
   - 46,000건을 루프 도는 정책기업 수집기(`import_policy_company.py`)는 매일 돌 필요가 없으므로, 매일 새벽 구동에서 **매주 일요일 새벽 05:35 (`35 5 * * 0`)**로 빈도를 하향 적용했습니다.
   - 이를 통해 평일 새벽에 종합쇼핑몰 수집 배치 등`과 데이터베이스를 두고 벌이는 락 경합 문제를 근본적으로 예방했습니다.
4. **오늘 자 수동 데이터 보정**:
   - 오늘 발생한 락 및 타임아웃으로 인한 누락을 채워두기 위해 수동으로 마스터 이관 배치를 실행하여 `success` 이력을 기록했고, `mas_api_incremental` 성공 기록 또한 `etl_job_log`에 수동으로 정상 이식해 두어 내일 아침 감시 프로그램 가동 시 어떠한 경보 문자도 오지 않도록 조치했습니다.
