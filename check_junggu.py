...
def find_agency_stats(keyword, start_dt, end_dt):
    ...
    def calc_stats(table_name, date_col, is_shop=False, use_location_filter=False):
        if is_shop:
            df = pd.read_sql(f"SELECT dlvrReqNo, dlvrReqChgOrd, prdctSno, dminsttCd, prdctAmt, cntrctCorpBizno FROM {table_name} WHERE {date_col} >= '{start_dt}' AND {date_col} <= '{end_dt}'", conn_pr)
            ...
            df = pd.read_sql(f"SELECT untyCntrctNo, cntrctInsttCd, totCntrctAmt, thtmCntrctAmt, corpList, dminsttList, cntrctRefNo{extra} FROM {table_name} WHERE {date_col} >= '{start_dt}' AND {date_col} <= '{end_dt}'", conn_pr)
...
if __name__ == '__main__':
    find_agency_stats('부산광역시 중구', '2026-01-01', '2026-01-31')
