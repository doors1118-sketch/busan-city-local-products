...
3. **서버 디스크 사용량 임계치(72%) 도달 및 장기 누적 리스크**
   * **리스크:** 현재 서버의 메인 루트 디스크(/)는 총 `50GB` 중 `34GB`가 사용 중이며 남은 공간은 `14GB` (사용률 **72%**)입니다. 당장은 수년간 데이터 적재에 무리가 없으나, 매일 쌓이는 모니터링 DB 로컬 백업본(약 140MB/회) 및 AI 챗봇의 대용량 QA 테스트 로그(약 5.1GB 점유 중) 등이 장기적으로 누적되면 디스크 꽉 참(Disk Full) 에러를 유발해 DB 수집 자체가 일시 중단될 수 있습니다.
   * **대응:** 
     1. 앞선 가이드에 따라 원격 NCP Object Storage 백업 인증키를 등록하여 원격 백업이 활성화되면, 로컬 백업 파일 보존 기한을 7일에서 3일 이하로 단축하여 스토리지 여유를 확보합니다.
     2. `/opt/advisor/app/data/qa_test_logs/` 디렉토리 아래의 챗봇 QA 로그 등 비필수 로그를 주기적으로 정리하거나 Logrotate 설정을 도입해야 합니다.
