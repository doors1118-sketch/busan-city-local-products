...
print(f"[{bgn_dt} ~ {end_dt}] 공사현황 API 전체 다운로드를 시작합니다...")
...
while page_no <= total_pages:
    ...
    try:
        ...
        if header.get('resultCode') != '00':
            print(f"API Error at page {page_no}: {header.get('resultMsg')}")
            break
        ...
        if page_no == 1:
            ...
            print(f"> 총 데이터 건수: {total_count}건, 예상 페이지 수: {total_pages}장")
            
        print(f"  - {page_no}/{total_pages} 페이지 수집 완료 ({len(items)}건)")
...
if all_items:
    ...
    # 컬럼명이 너무 많으므로 대표적인것만 확인
    print(f"\n데이터 프레임 형태: {df.shape}")
    
    # 엑셀 저장
    df.to_excel(output_filename, index=False)
    print(f"\n성공적으로 [ {output_filename} ] 에 {len(df)}건의 전체 데이터가 저장되었습니다!")
else:
